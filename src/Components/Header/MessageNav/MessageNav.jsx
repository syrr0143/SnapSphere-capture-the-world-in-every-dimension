    ...
    return (
        <div className='w-full h-full relative'>
            <Link to="/" className='mb-10 px-2 lg:block md:hidden sm:hidden hidden'>
                <img src={InstagramIcon} alt="Instagram Logo" className='w-8 h-auto' />
            </Link>
            ...
            <nav className="flex h-auto flex-col items-center justify-center lg:w-12 lg:flex-col lg:items-start lg:justify-start ">
                {sidebarItems.map((item, index) => (
                    <Link key={index} to={item.link} className="flex  p-3 relative bg-transparent hover:bg-gray-800/75 rounded-lg ease-out duration-500 group items-center justify-center lg:justify-start w-14 h-14 lg:w-auto lg:h-14">
                        <img
                            src={item.icon}
                            alt={item.name}
